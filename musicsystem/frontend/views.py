...
def free_streaming(request):
    podcast = Podcast.objects.filter().all()
...
def admin_delete(request,id, slug):
    if slug == 'gallery':
        ...
        Podcast.objects.filter(id=id).delete()
        messages.error(request, 'Music file deleted!')
...
def admin_podcast(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            data_record = form.cleaned_data
            podcast = Podcast(
                streaming_file=fs.url(file_name),
                text=data_record['text'],
            )
            podcast.save()
            messages.success(request, 'Mp3 added successfully!')
            return redirect('/admin-podcast')
    context = {'form': form,'podcast': Podcast.objects.filter()}
    return render(request, 'admin-podcast.html', context )